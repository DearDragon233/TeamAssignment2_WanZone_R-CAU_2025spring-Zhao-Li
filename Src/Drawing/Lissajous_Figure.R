# 自动检查并安装必要的包
required_packages <- c("ggplot2", "minpack.lm", "signal")
new_packages <- required_packages[!(required_packages %in% installed.packages()[, "Package"])]
if (length(new_packages)) install.packages(new_packages)
lapply(required_packages, library, character.only = TRUE)

# 1. 加载数据
load("Data/Processed/TIF_2DMatrix.RData")  # 加载 RData 文件

# 2. 选择两个纬度索引（例如纬度索引 5000 和 10000）

lat_index1 <- 100
lat_index2 <- 50

# 3. 提取对应经度上的 value 变化
x_data <- mat[, lat_index1]  # 第一个纬度对应的列
y_data <- mat[, lat_index2]  # 第二个纬度对应的列
